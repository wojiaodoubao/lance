...
use lance_namespace_impls::partition::{
    parse_filter_expr_from_sql, PartitionField, PartitionSpec, PartitionTable,
    PartitionedNamespace, PartitionedNamespaceBuilder,
};
...
fn create_partitioned_namespace_internal(
    ...
    if !properties.contains_key("root")
        && let Some(location) = properties.get("location").cloned()
    {
    ...
    let schema_json = properties.get("schema").cloned();
    let partition_spec_json = properties.get("partition_spec").cloned();
    ...
    let mut builder = PartitionedNamespaceBuilder::new(location).directory(directory);

    if let Some(schema_json) = schema_json {
        let json_schema: JsonArrowSchema = serde_json::from_str(&schema_json).map_err(|e| {
            Error::input_error(format!(
                "Invalid Arrow schema JSON in configProperties['schema']: {}",
                e
            ))
        })?;
        let arrow_schema = convert_json_arrow_schema(&json_schema).map_err(|e| {
            Error::input_error(format!(
                "Invalid Arrow schema in configProperties['schema']: {}",
                e
            ))
        })?;
        builder = builder.schema(arrow_schema);
    }

    if let Some(partition_spec_json) = partition_spec_json {
        let json_partition_spec: JsonPartitionSpec = serde_json::from_str(&partition_spec_json)
            .map_err(|e| {
                Error::input_error(format!(
                    "Invalid partition spec JSON in configProperties['partition_spec']: {}",
                    e
                ))
            })?;
        let partition_spec = PartitionSpec::from_json(&json_partition_spec).map_err(|e| {
            Error::input_error(format!(
                "Invalid partition spec in configProperties['partition_spec']: {}",
                e
            ))
        })?;
        builder = builder.partition_spec(partition_spec);
    }
    let ns = RT.block_on(builder.build()).map_err(|e| {
        Error::runtime_error(format!("Failed to build PartitionedNamespace: {}", e))
    })?;
