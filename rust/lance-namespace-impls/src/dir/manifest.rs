...
use lance::dataset::{builder::DatasetBuilder, NewColumnTransform, ReadParams, WriteParams};
...
impl ManifestNamespace {
    ...
    pub async fn create_namespace_extended(
        ...
        let object_id = namespace_id.join(crate::dir::manifest::DELIMITER);
        let existing = self
            .query_manifest(
                col("object_id")
                    .eq(lit(object_id.clone()))
                    .and(col("object_type").eq(lit("namespace"))),
            )
            .await?;
    ...
    for (k, v) in map.iter() {
        ...
        if let Some(col_name) = k.strip_prefix(EXTENDED_PREFIX) {
            if excluded.contains(&col_name) {
                return Err(Error::InvalidInput {
                    source: format!("Column {} is preserved.", col_name).into(),
                    location: location!(),
                });
            }
...
fn merge_extended_record_and_properties(
    ...
    let batch = RecordBatch::try_new_with_options(schema, columns, &options).map_err(|e| {
        Error::io(
...
fn build_extended_column_array(
    ...
    let key = format!("{}{}", crate::dir::manifest::EXTENDED_PREFIX, col_name);
    let null_scalar = ScalarValue::try_from(data_type).map_err(|e| Error::IO {
        source: box_error(std::io::Error::other(format!(
...
fn scalar_from_record(
    ...
    ScalarValue::try_from_array(col.as_ref(), 0)
        ...
        .map_err(|e| Error::Internal {
            message: format!("Failed to convert column '{}' to scalar: {}", col_name, e),
            location: location!(),
        })
}
