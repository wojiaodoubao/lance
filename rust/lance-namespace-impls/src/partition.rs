...
use crate::dir::manifest::{EXTENDED_PREFIX, ManifestObject, scalar_to_str};
...
use crate::{DirectoryNamespace, ManifestNamespace, context::DynamicContextProvider};
...
use lance::deps::datafusion::prelude::{SessionContext, col, lit};
...
impl PartitionedNamespace {
    ...
    pub async fn plan_scan(&self, filter: &Expr) -> Result<Vec<(PartitionTable, Expr)>> {
        ...
        for table in &self.table_specs {
            ...
            let objects = self
                .manifest
                .query_manifest_expr(table_filter.and(manifest_pred))
                .await?;
    ...
    pub async fn resolve_or_create_partition_table(
        ...
        // Concurrent writers (e.g. Spark tasks) may race to create the same partition table.
        // Some races surface as retryable commit conflicts from lower layers.
        ...
        for _attempt in 0..20 {
            ...
            match self.create_partition_table(&spec, &partition_values).await {
                ...
                Err(e) => {
                    // One last resolve attempt.
                    return if let Some(table) = self
                        .resolve_partition_table(&spec, &partition_values)
                        .await?
                    {
                        Ok(table)
        ...
        // Retries exhausted. If another writer won the race, resolve should now succeed.
        if let Some(table) = self
            .resolve_partition_table(&spec, &partition_values)
            .await?
        {
        ...
        // Since the latest error is a retryable commit conflict, return TooMuchWriteContention.
        Err(last_retryable.unwrap_or_else(|| {
            Error::too_much_write_contention(
                "Failed to resolve_or_create_partition_table after retries",
            )
        }))
    }
    ...
    async fn create_partition_table(
        ...
        if let Err(e) = self
            ...
            .await
        {
            // Concurrency: if a competitor created it, resolve and return.
            if let Some(table) = self.resolve_partition_table(spec, partition_values).await? {
                return Ok(table);
            }
            // Otherwise, propagate the underlying error so callers can decide whether to retry.
            return Err(e);
        }
    ...
    async fn resolve_partition_table(
        ...
        let objects = self.manifest.query_manifest_expr(table_expr).await?;
    ...
    pub fn commit(
        ...
        Err(Error::internal(
            "PartitionedNamespace.commit is not implemented",
        ))
    }
    ...
    pub async fn tables(&self) -> Result<Vec<PartitionTable>> {
        let objects = self
            .manifest
            .query_manifest_expr(col("object_type").eq(lit("table")))
            .await?;
    ...
    pub(crate) async fn force_sink_table_spec(&self, table: &TableSpec) -> Result<()> {
        ...
        let schema_json = serde_json::to_string(&json_schema)
            .map_err(|e| Error::internal(format!("Failed to serialize schema: {}", e)))?;

        let spec_json = serde_json::to_string(&table.partition_spec.to_json()?)
            .map_err(|e| Error::internal(format!("Failed to serialize partition spec: {}", e)))?;
        ...
        if let Err(e) = self.create_namespace(create_req).await {
            ...
            match load_table_spec(&self.directory, table.id).await {
                Ok(Some(existed_table)) => {
                    ...
                    {
                        return Err(Error::invalid_input(format!(
                            "A table spec with id {} already exists and contains different schema or partition spec",
                            existed_table.id
                        )));
                    }
                }
    ...
    pub fn type_promotion(&self, _column: &str, _new_type: &DataType) -> Result<ArrowSchema> {
        Err(Error::internal(
            "PartitionedNamespace.type_promotion is not implemented",
        ))
    }
    ...
    fn latest_table_spec(&self) -> Result<&TableSpec> {
        let table = self
            .table_specs
            .last()
            .ok_or_else(|| Error::internal("Could not find any table spec"))?;
        Ok(table)
    }
...
/// Is the error a retryable transaction commit error.
///
/// When commit transaction, the basic retryable errors are:
/// * Error::RetryableCommitConflict (retryable semantic error)
/// * Error::TooMuchWriteContention  (retryable semantic error or timeout error)
/// * Error::CommitConflict          (commit conflict error)
///
/// In lance namespace the retryable basic errors are transformed to NamespaceError::Throttled.
///
fn is_retryable_commit_conflict(e: &Error) -> bool {
    match e {
        Error::RetryableCommitConflict { .. } => true,
        Error::CommitConflict { .. } => true,
        Error::TooMuchWriteContention { .. } => true,
        Error::Namespace { source, .. } => source
            .downcast_ref::<NamespaceError>()
            .is_some_and(|e| matches!(e, NamespaceError::Throttled { .. })),
        _ => false,
    }
}
...
async fn load_table_spec(dir: &DirectoryNamespace, v: i32) -> Result<Option<TableSpec>> {
    ...
    if let Some(schema_json) = props.get(NS_PROP_SCHEMA) {
        let json_schema: JsonArrowSchema =
            serde_json::from_str(schema_json.as_str()).map_err(|e| {
                Error::internal(format!(
...
async fn partition_values(
    ...
    for field in fields.iter() {
        ...
        let scalar = ScalarValue::try_from_array(&partition_value, 0).map_err(|e| {
            Error::internal(format!(
...
fn build_partition_extended_record(
    ...
    for (idx, f) in fields.iter().enumerate() {
        ...
        let scalar = if idx > max {
            ScalarValue::try_from(&f.result_type).map_err(|e| {
                Error::internal(format!(
            ...
            partition_values.get(idx).cloned().ok_or_else(|| {
    ...
    RecordBatch::try_new(schema, arrays)
        .map_err(|e| Error::internal(format!("Failed to create extended record batch: {}", e)))
}
...
impl PartitionedNamespaceBuilder {
    ...
    pub async fn build(self) -> Result<PartitionedNamespace> {
        ...
        let schema = self.schema.ok_or_else(|| {
            Error::invalid_input("schema is required when creating a new partitioned namespace")
        })?;
        let partition = self.partition_spec.ok_or_else(|| {
            Error::invalid_input(
                "partition_spec is required when creating a new partitioned namespace",
            )
        })?;
    ...
    pub async fn load(self) -> Result<PartitionedNamespace> {
        ...
        if !tables.is_empty() {
            ...
            Err(Error::invalid_input(
                "PartitionedNamespace is not initialized",
            ))
        }
    }
    ...
    async fn open_directory(
            ...
            && (!credential_vendor_properties.is_empty() || context_provider.is_some())
        {
            return Err(Error::invalid_input(
                "Cannot set credential_vendor/context_provider when directory is explicitly provided",
            ));
        }
    ...
    async fn create_new(
        ...
        if partition.id != 1 {
            return Err(Error::invalid_input("initial partition spec id must be 1"));
        }
...
fn first_id(schema: &ArrowSchema, source_ids: &[i32]) -> Result<(FieldRef, String)> {
    let id = *source_ids
        .first()
        .ok_or_else(|| Error::invalid_input("source_ids should have at least one element"))?;

    let (col, field) = schema
        .path_and_field_by_id(id)?
        .ok_or_else(|| Error::invalid_input(format!("Field id {} not found in schema", id)))?;
...
impl PartitionField {
        ...
        let transform = json
            ...
            .map(|t| {
                let result = match t.r#type.as_str() {
                    ...
                    other => {
                        return Err(lance_core::Error::namespace(format!(
    ...
    pub async fn value(&self, record: &RecordBatch) -> Result<Arc<dyn Array>> {
        if record.num_rows() != 1 {
            return Err(Error::invalid_input("record must contain exactly one row"));
        }
        ...
        let array = match (self.expression.as_ref(), self.transform.as_ref()) {
            ...
            _ => {
                return Err(Error::internal(
                    "expression and transform can't both be set or unset",
                ));
            }
        };
        ...
        if array.is_empty() {
            return Err(Error::internal(
                "partition value is empty, the expression might be invalid",
            ));
        }
        Ok(array)
    }
    ...
    pub async fn partition_prune(&self, schema: ArrowSchema, cdf: &Vec<Vec<Expr>>) -> Result<Expr> {
        ...
        for source_id in &self.source_ids {
            let (path, _) = schema.path_and_field_by_id(*source_id)?.ok_or_else(|| {
                Error::invalid_input(format!("Field id {} not found in schema", source_id))
            })?;
        ...
        for clause in cdf {
            ...
            if all_relevant_are_eq
                && covers_all_sources
                && let Some(rb) = build_single_row_record_batch(&schema, &eq_map)
            {
...
async fn parse_partition_value_from_expr(
    ...
    let partition_batch = records
        .first()
        .ok_or_else(|| Error::internal("expect one row of partition value but got nothing"))?;
...
async fn parse_partition_value_from_transform(
    ...
    let expr = match transform {
        ...
        PartitionTransform::Bucket { num_buckets } => {
            if *num_buckets <= 0 {
                return Err(Error::invalid_input(format!(
                    "num_buckets must be positive, got {}",
                    num_buckets
                )));
            }
            format!(
            ...
            let cols: Vec<String> = ids
                ...
                .map(|id| {
                    schema
                        ...
                        .map(|(p, _)| p)
                        .ok_or_else(|| {
                            Error::invalid_input(format!("Field id {} not found in schema", id))
                        })
                })
                .collect::<Result<Vec<_>>>()?;
            if cols.is_empty() {
                return Err(Error::invalid_input(
                    "source_ids should have at least one element",
                ));
            }
        ...
        PartitionTransform::Truncate { width } => {
            if *width <= 0 {
                return Err(Error::invalid_input(format!(
                    "truncate width must be positive, got {}",
                    width
                )));
            }
...
fn create_partition_namespace_id(
    ...
    for (i, v) in partition_values.iter().enumerate() {
        let v_str = scalar_to_str(v)?.ok_or_else(|| {
            Error::invalid_input(format!(
                "partition value with index {} should not be null",
                i
            ))
        })?;
        buf.push(v_str);
...
mod tests {
    ...
    use arrow_schema::{Field, Field as ArrowField, Fields, Schema as ArrowSchema, Schema};
    use lance_arrow::LANCE_FIELD_ID_META_KEY;
    ...
    impl PartitionedNamespace {
        ...
        async fn resolve_partition_namespace(
            ...
            let mut ns_ids = vec![];
            let objects = self.manifest.query_manifest_expr(ns_expr).await?;
            ...
            if ns_ids.is_empty() {
                ...
                Err(Error::internal(format!(
                    "Found multiple partition namespace instance with the same spec id and partition values {:?}",
                    ns_ids
                )))
            }
        }
    ...
    async fn test_create_partition_table() {
        ...
        // Dataset schema doesn't carry field-id metadata. Normalize before comparing.
        assert_eq!(
            ensure_all_schema_field_have_id(ArrowSchema::from(ds.schema())).unwrap(),
            ns.schema().unwrap()
        );
    ...
    async fn create_ns_and_record(
        temp_path: &str,
        values: Vec<ScalarValue>,
    ) -> Result<Vec<(PartitionedNamespace, RecordBatch, Vec<ScalarValue>)>> {
        let col1 = ScalarValue::iter_to_array(vec![values.first().unwrap().clone()])?;
        let col2 = ScalarValue::iter_to_array(vec![values.get(1).unwrap().clone()])?;
        let mut col1_meta = HashMap::new();
        col1_meta.insert(LANCE_FIELD_ID_META_KEY.to_string(), "0".to_string());
        let mut col2_meta = HashMap::new();
        col2_meta.insert(LANCE_FIELD_ID_META_KEY.to_string(), "1".to_string());
        let schema = Arc::new(Schema::new(vec![
            Field::new("country", DataType::Utf8, true).with_metadata(col1_meta),
            Field::new("year", DataType::Int32, true).with_metadata(col2_meta),
        ]));
        let batch = RecordBatch::try_new(schema, vec![col1, col2])?;
        let mut ns_list: Vec<(PartitionedNamespace, RecordBatch, Vec<ScalarValue>)> = vec![];
        for _ in 0..8 {
            let ns = create_partitioned_namespace_country_year(temp_path).await;
            ns_list.push((ns, batch.clone(), values.clone()));
        }
        Ok(ns_list)
    }

    async fn create_missing_table_same_id(
        temp_path: &str,
        values_list: Vec<Vec<ScalarValue>>,
    ) -> Result<()> {
        let ns = create_partitioned_namespace_country_year(temp_path).await;
        let spec = ns.partition_spec().await.unwrap();
        ns.ensure_partition_fields_exists(&spec.fields).await?;

        let mut ns_list: Vec<(PartitionedNamespace, RecordBatch, Vec<ScalarValue>)> = vec![];
        for values in values_list.into_iter() {
            let ns_record_list = create_ns_and_record(temp_path, values).await?;
            ns_list.extend(ns_record_list);
        }

        // Execute concurrent create.
        let mut set = JoinSet::new();
        for (ns, batch, values) in ns_list.into_iter() {
            set.spawn(async move {
                let table = ns.resolve_or_create_partition_table(&batch).await.unwrap();
                (table, values)
            });
        }

        let mut table_values_res: Vec<(PartitionTable, Vec<ScalarValue>)> = vec![];
        while let Some(res) = set.join_next().await {
            table_values_res.push(res?);
        }

        for (created_table, values) in table_values_res.into_iter() {
            let table = ns.resolve_partition_table(&spec, &values).await?.unwrap();
            assert_eq!(table.id, created_table.id);
        }
    ...
    #[tokio::test]
    async fn test_resolve_or_create_missing_table_same_id() {
        let temp_dir = TempStdDir::default();
        let temp_path = temp_dir.to_str().unwrap();
        ...
        create_missing_table_same_id(temp_path, values)
            .await
            .unwrap();
    }

    async fn create_missing_partition_namespaces_same_id(
        temp_path: &str,
        values_list: Vec<Vec<ScalarValue>>,
    ) -> Result<()> {
        let ns = create_partitioned_namespace_country_year(temp_path).await;
        let spec = ns.partition_spec().await?;
        ns.ensure_partition_fields_exists(&spec.fields).await?;

        // Create multiple PartitionedNamespace instances per values group.
        let mut tasks: Vec<(PartitionedNamespace, Vec<ScalarValue>)> = Vec::new();
        for values in values_list.into_iter() {
            for _ in 0..8 {
                let ns_i = create_partitioned_namespace_country_year(temp_path).await;
                tasks.push((ns_i, values.clone()));
            }
        }
        ...
        let mut set = JoinSet::new();
        for (ns_i, values) in tasks.into_iter() {
        ...
        let mut created: Vec<(Vec<String>, Vec<ScalarValue>)> = Vec::new();
        while let Some(res) = set.join_next().await {
            created.push(res?);
        }

        for (created_id, values) in created.into_iter() {
            let expected = create_partition_namespace_id(&spec, &values)?;
            assert_eq!(created_id, expected);

            // Resolve partition namespace from __manifest by partition values.
            let leaf = ns
    ...
    async fn test_create_missing_partition_namespaces_concurrent_same_id() {
        ...
        create_missing_partition_namespaces_same_id(temp_path, values)
            .await
            .unwrap();
    }
