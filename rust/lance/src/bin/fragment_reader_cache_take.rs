...
async fn fragment_reader_take(
    ...
    for num_rows in [1000] {
        ...
        println!("Fragment Reader Cache Reader Take time costs: {:?}", time_cost);
    }
...
async fn bench_random_single_take_with_fragment() -> Result<(), lance_core::Error> {
...
async fn main() -> lance_core::Result<()> {
    bench_random_single_take_with_fragment().await
}