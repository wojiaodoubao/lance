...
public final class PartitionedNamespace implements LanceNamespace, Closeable {
  ...
  public Schema schema() {
    ensureOpen();
    Preconditions.checkNotNull(allocator, "allocator is null");
    byte[] schemaIpc = schemaNative(nativeHandle);
    try (ArrowStreamReader reader =
        new ArrowStreamReader(new ByteArrayInputStream(schemaIpc), allocator)) {
      return reader.getVectorSchemaRoot().getSchema();
    } catch (Exception e) {
      throw new RuntimeException("Failed to decode Arrow schema", e);
    }
  }

  byte[] schemaToIpc(Schema schema) {
  ...
  public PartitionSpec partitionSpec() {
    ensureOpen();
    String json = partitionSpecNative(nativeHandle);
    return fromJson(json, PartitionSpec.class);
  }

  /**
   * Update the table spec.
   *
   * @param schema new table schema
   * @param partitionFields new table partition spec
   * @return the latest table spec
   */
  public void updateTableSpec(Schema schema, List<PartitionField> partitionFields) {
    ensureOpen();
    Preconditions.checkNotNull(schema, "schema is null");
    Preconditions.checkNotNull(partitionFields, "partitionFields is null");
    updateTableSpecNative(nativeHandle, schema, partitionFields);
  }
  ...
  private native byte[] schemaNative(long handle);

  private native String planScanNative(long handle, String filter);

  private native String partitionSpecNative(long handle);

  private native void updateTableSpecNative(
      long handle, Schema schema, List<PartitionField> partitionFields);
